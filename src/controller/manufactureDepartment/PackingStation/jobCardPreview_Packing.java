...
public class jobCardPreview_Packing implements Initializable
{
    // Text field
    ...
    public void popUpEndDatePage(String jId, String fxmlPath, String orderId) 
    {
    try {
        FXMLLoader loader = new FXMLLoader(getClass().getResource(fxmlPath));
        root = loader.load();
        // change controller
        updateEndDate_Packing upEndDate = loader.getController();
        ...
        upEndDate.previeweJobCardDetails(jId, rQ, orderId);
    ...
    public void popUpStartJobPage (String oId, String jId,String opCode, String fxmlPath) 
    {
        ...
        root = loader.load();
        // change controller
        updateStartDate_Packing upStartDate = loader.getController();
        String rQ = eleJobCard.showRequiredQuantity(jId,getRequiredOrderQuantity);
        Stage stage = new Stage();
        Scene scene = new Scene(root);
            ...
            if (inUpdateStatusMode) {
                ...
                if (diff<300) {
                    if (row.getEnd_Date()==null&&row.getStart_Date()==null) 
                    {
                        ...
                        popUpStartJobPage(row.getOrder_Id(), row.getJc_Id(),row.getOp_Code(), "/fxml/manufactureDepartment/PackingStation/updateStartDate_Packing.fxml");   
                    } else {
                        if (row.getEnd_Date()==null) // Complete Job
                        {
                            // can complete job only
                            inUpdateStatusMode = true;
                            popUpEndDatePage(row.getJc_Id(),"/fxml/manufactureDepartment/PackingStation/updateEndDate_Packing.fxml",row.getOrder_Id() );
                        } else {
                            // continue;
                        }
                    }
