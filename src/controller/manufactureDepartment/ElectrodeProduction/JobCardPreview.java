...
//Program Name: JobCardPreview.java
...
public class JobCardPreview implements Initializable
{
    ...
    public void popUpEndDatePage(String jId, String fxmlPath) 
    {
        try {
            ...
            root = loader.load();
            updateEndDate upEndDate = loader.getController();
    ...
    public void popUpStartJobPage (String oId, String jId,String opCode, String fxmlPath) 
    {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource(fxmlPath));
            root = loader.load();
            updateStartDate upStartDate = loader.getController();
            String rQ = eleJobCard.showRequiredQuantity(jId,getRequiredOrderQuantity);
            Stage stage = new Stage();
            Scene scene = new Scene(root);
            ...
            if (inUpdateStatusMode) {
                ...
                if (diff<300) {
                    if (row.getEnd_Date()==null&&row.getStart_Date()==null) 
                    {
                        ...
                        popUpStartJobPage(row.getOrder_Id(), row.getJc_Id(),row.getOp_Code(), "/fxml/manufactureDepartment/ElectrodeProduction/updateStartDate.fxml");   
                    } else {
